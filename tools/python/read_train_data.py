"""
Module to read XNet train files 
"""
def read_train_data(file_name, debug=False, last = False, print_positions=False):
    """
    ...
    Outputs< zz, proton number of every species in network 
             ...
             y, mass fraction as a function of time, for every species
             yo, initial mass fraction, for every species
             ydot, initial mass fraction as a function of time, for every species
             timed time as a function of time
             temperqature, temperature (in GK) as a function of time
             t9o, initial temperature (in GK) 
             t9dot, initial temperature (in GK) as a function of time 
   ...
   #Read Run Settings
    ...
    # Size data arrays
        ...
        y           = np.zeros((kstmx_size,ny),dtype='float64')
        yo          = np.zeros((kstmx_size,ny),dtype='float64')
        ydot        = np.zeros((kstmx_size,ny),dtype='float64')
        t9o         = np.zeros(kstmx_size,dtype='float64')
        t9dot       = np.zeros(kstmx_size,dtype='float64')
        if print_positions:
    ...
    # Otherwise read data
               ...
               y[k,:]         =np.fromfile(file_id,dtype='float64',count=ny)
               yo[k,:]        =np.fromfile(file_id,dtype='float64',count=ny)
               ydot[k,:]      =np.fromfile(file_id,dtype='float64',count=ny)
               t9o[k]         =np.fromfile(file_id,dtype='float64',count=1)
               t9dot[k]       =np.fromfile(file_id,dtype='float64',count=1)
               record_length2 =np.fromfile(file_id,dtype='uint32',count=1)
               if debug:
    ...
    return zz, aa, y, time, temperature, density, timestep, edot, flx_end, yo, ydot, t9o, t9dot 
