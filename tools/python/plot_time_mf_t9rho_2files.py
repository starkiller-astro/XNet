def massfractiont9rhovtime2(datafile1, datafile2, end, num_plots = 2, num_species = 14, min_mf = .00000001, time_spacing = .2, h_ratio = [3, 1], zz_wanted = 'None', zz_wanted2 = 'None', aa_wanted = 'None', nuc_names_wanted = 'None'):
        ...
        if zz_wanted != 'None':
            if zz1[counter] in zz_wanted and aa_wanted == 'None': #Plot all isotopes of an element.
                if zz1[counter] == 1 and aa1[counter] == 1: #Explicitly plots protons and hydrogen.
                    ...
                    plt.plot(time1, xmf1[:, counter], color = colors[counter], label = nuc_name[counter])
    ...
    if nuc_names_wanted != 'None': #Sort through list to find mass fraction of named species, and plot.
        for counter in np.arange(0, len(nuc_names_wanted)):
            ...
            plt.plot(time1, xmf1[:, species_number], color = colors[species_number], label = nuc_name[species_number])
            box = ax1.get_position()
            ax1.set_position([box.x0, box.y0, box.width * 0.995, box.height])
            ax1.legend(loc = 'center left', bbox_to_anchor = (1, 0.5), fontsize = 10)
        ...
        if zz_wanted2 != 'None':
            if zz2[counter] in zz_wanted2 and aa_wanted == 'None':
                if zz[counter] == 1 and aa2[counter] == 1:
                    plt.plot(time2, xmf2[:, counter], color = 'r', label = nuc_name[counter], linestyle = '--')
                elif zz2[counter] == 1 and aa2[counter] == 2:
                    plt.plot(time2, xmf2[:, counter], color = 'b', label = nuc_name[counter], linestyle = '--')
                else:
                    plt.plot(time2, xmf2[:, counter], color = colors[counter], label = nuc_name[counter], linestyle = '--')
            ...
            if np.amax(xmf2[:, counter]) >= min_mf:
                plt.plot(time2, xmf2[:, counter], color = colors[counter], linestyle = '--')
    ...
    if nuc_names_wanted != 'None':
        for counter in np.arange(0, len(nuc_names_wanted)):
            species_number = nuc_name.index(nuc_names_wanted[counter])
            plt.plot(time2, xmf2[:, species_number], color = colors[species_number], label = nuc_name[species_number], linestyle = '--')
    ...
    temp_line2 = plt.plot(time2, temperature2, color = 'r', linestyle = '--', label = "Temperature")
    plt.yscale('linear')
    plt.ylim(0, 10)
    ...
    dens_line2 = ax3.plot(time2, density2, color = 'b', linestyle = '--', label = "Density")
    plt.yscale('log')
    plt.ylim(10E0, 10E10)
