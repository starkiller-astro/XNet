...
!-----------------------------------------------------------------------------
      use nuc_number
      integer, parameter :: nrm1=5000,nrm2=10000,nrm3=50
...
!  Convert nuclear names to indicies
          Do i=1,6 ! run through the 6 possible  reactants and products
...
!  Other weak rates not covered above (iec=4)
          If(desc==lec) Then
              iec=1
          Elseif(desc==lrat) Then
            If(zz(ni(1))>zz(ni(2)).and.zz(ni(1))/=1) Then
              iec=3
            Else
              iec=2
            Endif
          Elseif(desc==betp.or.desc==betm.or.desc==btyk.or.
     &           desc==bec) Then
            iec=4
          Else
            iec=0
          Endif
          write(980,*) name(1),name(2),desc,iec
...
! ,iec,ier,iev,p,q
          Else
            ...
            If(nreactant(krt)==2) Then
             ...
             write(21,*) 'theresa',nreactant(krt)
     & ,n,(nname(ni(i)),i=1,5)
!,iec,ier,iev,p,q
...
!  For each nucleus, register each single reactant reaction which involves it.
      ...
      Do i=1,ny
        ...
        Do j=1,3 ; Do k=la(j),le(j) ;  Do l=1,nnucr(j)
          Write(900,*) j,k,l,n1(l,k)
          If(n1(l,k)==i.and.an1(l,k)/=0.) Then
            n=n+1
            a1(n)=an1(l,k)
            mu1(n)=k
            write(3)  a1(n),mu1(n)
           write(22,*) i,n, a1(n),mu1(n),nname(n1(l,k))
          Endif
        Enddo ; Enddo ; Enddo
        l1e(i)=n
      Enddo
...
!  Output reaction matching
      ...
      Write(600,*) nflx(:,1:mflx)
      Write(600,*) '--------------------'
      Write(600,*) qflx(1:mflx)
      Write(600,*) '--------------------'
      Write(600,*) iwflx(1:mflx)
      Write(600,*) '--------------------'
      Write(600,*) descx(1:mflx)
      Write(600,*) '--------------------'
      Write(600,*)  ifl1(1:le(3))
      Write(600,*) '--------------------'
      Write(600,*)  ifl1(1:le(3))
       DO i=1,le(3)
       Write(600,*) ifl1(i)
