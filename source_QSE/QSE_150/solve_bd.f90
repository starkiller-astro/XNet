...
! Calculate the right hand side
  rhs = dt*ydot
  If(idiag>=2) Write(lun_diag,"(i4,15es10.3)") 0,ttemp,yt
...
! Advance time
  ttemp=ttemp+dt
  If(idiag>=2) Write(lun_diag,"(i4,15es10.3)") 1,ttemp,yt
...
! Advance time
    ttemp=ttemp+dt
    If(idiag>=2) Write(lun_diag,"(i4,15es10.3)") step_loop,ttemp,yt
    Call yderiv
  EndDo
...
! Use yout for the final population
  yt = yt + dy
  yout = yt
  If(idiag>=2) Write(lun_diag,"(i4,15es10.3)") step_loop,ttemp,yout
