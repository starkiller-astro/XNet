...
| `reset_timers()` reset of public `timer_output` | Unit | characterized | Exact zero after a nonzero sentinel, required by [issue #55](https://github.com/jaharris87/XNet/issues/55) and its [maintainer disposition](https://github.com/jaharris87/XNet/issues/55#issuecomment-5171650692) | GNU Fortran, serial, normal dependency-light unit configuration | `serial-unit-tests` | Implementation contract only; later summaries do not retain `Output` from an earlier reset epoch. |
