...
Module xnet_ffn
  ...
  Real(dp), Allocatable :: ffn_qval(:), fermi_factor(:)
...
Contains
  ...
  Subroutine read_ffn_data_logft(nffn,data_dir)
    ...
    !-----------------------------------------------------------------------------------------------
    Use nuclear_data, Only: index_from_name,zz
    Use xnet_constants, Only: pi,a_fine
    ...
    Allocate (fermi_factor(nffn))

    Open(newunit=lun_ffn, file=trim(data_dir)//"/netweak", status='old')
    Do i = 1, nffn
      Read(lun_ffn,*)  nuc1, nuc2,desc, ffn_qval(i)

      Call index_from_name(nuc1,inuc)
      If (inuc <=0) Then
          Write(lun_diag,*) "Nucleus in netweak not found!",nuc1
      Endif   


      If (desc=='ft+') Then 
           has_logft(i)=2 !positron capture
           fermi_factor(i)=exp(pi*a_fine*zz(inuc)) ! Estimate from Schwab et al. (2015) Eq. A13
  ...
  Subroutine ffn_rate_logft(nffn,t9,ene,rf,dlnrfdt9,mask_in)
    !-----------------------------------------------------------------------------------------------
          ...
          If (has_logft(k) > 0) then
           ...
           !  write(*,*) "rf_beta: ",rf_beta
             ...
             If (phasei< 1d-30 .or. rf_ec<1.d-30 ) Then
                     rf_ec = 0.0
             Else
                     rf_ec=dlog(2.0d0)*phasei*fermi_factor(k)/rf_ec ! turn into rate
             EndIf
             ...
             Else
               ...
               Else
                  ...
                  drec_dt = rf_ec * ( -ln_10 * ( rdene*dr2_ec + (1.0d0-rdene)*dr1_ec) /dt9 + &
                        & dphase_dt/phasei*fermi_factor(k) )
