...
!===============================================================================
  ...
  Integer :: k,idiag0
  Real(8), Dimension(ny) :: yrhs,dy,reldy
  Real(8) :: t9rhs,dt9,relt9
...
! The Newton-Raphson iteration occurs for at most kitmx iterations.  
  Do kit=1,kitmx
    ...
    If(kit>1) Then
      If(iheat>0) Call cross_sect
      Call yderiv

! Rebuild the jacobian and every ijac iterations
...
&       (nname(k),yt(k),dy(k),reldy(k),(aa(k)*dy(k)),(aa(k)*yt(k)),k=1,ny)
    ...
    If(iheat>0) Then
      ...
      Else
        relt9=0.0
      EndIf
      If(idiag>=3) Write(lun_diag,"(a2,i5,i3,5x,2es12.4,5x,es12.4)") &
&       'dT9',kstep,kit,dt9,t9t,relt9
    Else
      relt9=0.0
    EndIf
...
! If converged, exit NR loop
    If(abs(testn)<=toln.and.relt9<tolt9) Exit
