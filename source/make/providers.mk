...
ifeq ($(GPU_MODE),ON)
  ifeq ($(GPU_BACKEND),CUDA)
    ...
    GPU_PROVIDER_SRC += $(XNET_DIR)/cudaf.F90
    ...
    ifeq ($(GPU_LAPACK_VER),CUBLAS)
      ...
      GPU_PROVIDER_SRC += $(addprefix $(XNET_DIR)/,cublasf.F90 cusparsef.F90 cusolverf.F90)
      ...
      GPU_PROVIDER_SRC += $(addprefix $(XNET_DIR)/,magmaf.F90 cublasf.F90 cusparsef.F90 cusolverf.F90)
    endif
    ...
    GPU_PROVIDER_SRC += $(addprefix $(XNET_DIR)/,hipf.F90 hipblasf.F90 rocblasf.F90 rocsparsef.F90 rocsolverf.F90)
  endif

  ifeq ($(OPENACC_MODE),ON)
    GPU_DEFINES += -DXNET_OACC
    GPU_PROVIDER_SRC += $(XNET_DIR)/openaccf.F90
    GPU_DIRECTIVE_FLAGS := $(OPENACC)
  else ifeq ($(OPENMP_OL_MODE),ON)
    GPU_DEFINES += -DXNET_OMP_OL
    GPU_PROVIDER_SRC += $(XNET_DIR)/openmpf.F90
...
ifeq ($(EOS),STARKILLER)
  ...
  EOS_SRC := $(abspath $(STARKILLER_HELMHOLTZ_PATH))/actual_eos.F90 \
             ...
             $(XNET_DIR)/xnet_eos_starkiller.F90
...
JAC_SRC := $(XNET_DIR)/xnet_jacobian_$(MATRIX_SOLVER).F90
