...
c
      if ( rep .eq. 'coordinate' ) then
         nnzreq=nnz
         write(unit=ounit,fmt='(2I12,I26)') rows,cols,nnz
         if ( field .eq. 'integer' ) then
            do 10 i=1,nnzreq
 ...
 10         continue
         elseif ( field .eq. 'real' ) then
            do 20 i=1,nnzreq
               write(unit=ounit,fmt='(2I12,D26.16)')indx(i),jndx(i),
     *                                              rval(i)
 20         continue
         elseif ( field .eq. 'complex' ) then
            do 30 i=1,nnzreq
...
c        symm .eq. 'skew-symmetric' 
         ...
         write(unit=ounit,fmt='(2I12)')rows,cols
         if ( field .eq. 'integer' ) then
            do 50 i=1,nnzreq
 ...
 50         continue
         elseif ( field .eq. 'real' ) then
            do 60 i=1,nnzreq
               write(unit=ounit,fmt='(D26.16)')rval(i)
 60         continue
         else
c        field .eq. 'complex' 
            do 70 i=1,nnzreq
