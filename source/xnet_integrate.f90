...
Contains
  ...
  Subroutine timestep(kstep,mask_in)
    ...
    Real(dp), Parameter :: changest = 0.01
    ...
    !$acc copyin(mask) &
    !$acc create(mask_init,dtherm,rtau_y,tdel_dy,tdel_dt9,tdel_stop)

    ! Retain old values of timestep and thermo and calculate remaining time
    !$acc parallel loop gang async(tid) &
    !$acc present(mask,mask_init,tdel,tdel_old)
    Do izb = zb_lo, zb_hi
      If ( mask(izb) ) Then
    ...
    !$acc parallel loop gang async(tid) &
    !$acc present(mask,mask_init,t,tt,tstop,y,yo,ydot,t9,t9o,t9dot,ints,intso,nh, &
    !$acc         tdel,tdel_next,tdel_old,tdelstart,tdel_stop,tdel_dy,tdel_dt9) &
    !$acc private(tdel_init,rtau_y,rtau_t9,changey,changet9)
    Do izb = zb_lo, zb_hi
      If ( mask(izb) ) Then
        ...
        If ( mask_init(izb) ) Then
          ...
          tdel_init = min(changest*tdelstart(izb),tdel_stop(izb))
          If ( nh(izb) > 2 ) Then
        ...
        If ( tdel_old(izb) >= 0.0 ) Then
          ...
          !$acc loop vector
          Do k = 1, ny
            If ( y(k,izb) > yacc ) Then
              If ( mask_init(izb) ) Then
                ! Calculate timescale directly from derivatives.
                rtau_y(k) = abs(ydot(k,izb)/y(k,izb))
              Else
                ! Calculate timescale from changes in last timestep.
                rtau_y(k) = abs((y(k,izb)-yo(k,izb))/y(k,izb)) / tdel_old(izb)
              EndIf
            Else
              rtau_y(k) = 0.0
            EndIf
    ...
    If ( idiag >= 2 ) Then
      Do izb = zb_lo, zb_hi
        If ( mask(izb) ) Then
          ...
          !$acc update wait(tid) &
          !$acc host(tdel(izb),tdel_old(izb),tdel_stop(izb),tdel_next(izb), &
          !$acc      tdel_dy(izb),tdel_dt9(izb),ints(izb))
      ...
      If ( idiag >= 2 ) Then
        Do izb = zb_lo, zb_hi
          If ( mask(izb) .and. nh(izb) > 1 ) Then
            izone = izb + szbatch - zb_lo
            !$acc update wait(tid) &
            !$acc host(dtherm(izb),tdel(izb),t9t(izb),rhot(izb))
            ...
            Write(lun_diag,"(a5,2i5,2es24.16)") 'T9del',kstep,izone,tdel(izb),dtherm(izb)
    ...
    !$acc delete(mask,mask_init,dtherm,rtau_y,tdel_dy,tdel_dt9,tdel_stop)
  ...
  Subroutine yderiv(mask_in)
    ...
    !$acc present(mask,la,le,ydot,yt,b1,b2,b3,b4,mu1,mu2,mu3,mu4, &
    !$acc         a1,a2,a3,a4,csect1,csect2,csect3,csect4, &
    !$acc         n11,n21,n22,n31,n32,n33,n41,n42,n43,n44) &
    !$acc private(la1,la2,la3,la4,le1,le2,le3,le4,s1,s2,s3,s4)
    Do izb = zb_lo, zb_hi
      Do i0 = 1, ny
        If ( mask(izb) ) Then
          la1 = la(1,i0)
    ...
    If ( iheat > 0 ) Then
      ...
      !$acc parallel loop gang async(tid) &
      !$acc present(mask,ydot,t9dot,cv,mex) &
      !$acc private(sdot)
      Do izb = zb_lo, zb_hi
        If ( mask(izb) ) Then
          sdot = 0.0
    ...
    If ( idiag >= 5 ) Then
      Do izb = zb_lo, zb_hi
        If ( mask(izb) ) Then
          izone = izb + szbatch - zb_lo
          !$acc update wait(tid) &
          !$acc host(yt(:,izb),t9t(izb),ydot(:,izb),t9dot(izb),b1(:,izb),b2(:,izb),b3(:,izb),b4(:,izb))
          ...
          Do i0 = 1, ny
            ...
            EndIf
            Write(lun_diag,"(a4,a5,2es24.16,4es11.3)") 'YDOT',nname(i0),yt(i0,izb),ydot(i0,izb),s1,s2,s3,s4
          EndDo
          If ( iheat > 0 ) Write(lun_diag,"(5x,a5,2es24.16)") '   T9',t9t(izb),t9dot(izb)
        EndIf
      EndDo
      Flush(lun_diag)
    EndIf
  ...
  Subroutine cross_sect(mask_in)
    ...
    !$acc parallel loop gang async(tid) &
    !$acc present(mask,ene,t09,dt09,t9t,rhot,yet)
    Do izb = zb_lo, zb_hi
      If ( mask(izb) ) Then
    ...
    Else
      !$acc parallel loop gang async(tid) &
      !$acc present(mask,t09,dt09,rc1,rc2,rc3,rc4,h1,h2,h3,h4,dh1dt9,dh2dt9,dh3dt9,dh4dt9)
      Do izb = zb_lo, zb_hi
        If ( mask(izb) ) Then
          !$acc loop vector &
          !$acc private(lambda1,dlam1dt9)
          Do k = 1, nr1
            lambda1 = 0.0
            dlam1dt9 = 0.0
            !$acc loop seq &
            !$acc reduction(+:lambda1,dlam1dt9)
            Do j = 1, 7
          ...
          EndDo
          !$acc loop vector &
          !$acc private(lambda2,dlam2dt9)
          Do k = 1, nr2
            lambda2 = 0.0
            dlam2dt9 = 0.0
            !$acc loop seq &
            !$acc reduction(+:lambda2,dlam2dt9)
            Do j = 1, 7
          ...
          EndDo
          !$acc loop vector &
          !$acc private(lambda3,dlam3dt9)
          Do k = 1, nr3
            lambda3 = 0.0
            dlam3dt9 = 0.0
            !$acc loop seq &
            !$acc reduction(+:lambda3,dlam3dt9)
            Do j = 1, 7
          ...
          EndDo
          !$acc loop vector &
          !$acc private(lambda4,dlam4dt9)
          Do k = 1, nr4
            lambda4 = 0.0
            dlam4dt9 = 0.0
            !$acc loop seq &
            !$acc reduction(+:lambda4,dlam4dt9)
            Do j = 1, 7
    ...
    !$acc parallel loop gang async(tid) &
    !$acc present(mask,rhot,iweak,gg,dlngdt9,ene,iffn,rffn,dlnrffndt9,innu,rnnu,ktot, &
    !$acc         n1i,n2i,n3i,n4i,iwk1,iwk2,iwk3,iwk4,irev1,irev2,irev3,irev4, &
    !$acc         h1,h2,h3,h4,dh1dt9,dh2dt9,dh3dt9,dh4dt9,csect1,csect2,csect3,csect4, &
    ...
    Do izb = zb_lo, zb_hi
      If ( mask(izb) ) Then
        ...
        !$acc loop vector &
        !$acc private(rpf1,dlnrpf1dt9,p1,s1)
        Do k = 1, nr1
        ...
        !$acc loop vector &
        !$acc private(rpf2,dlnrpf2dt9,p2,s2)
        Do k = 1, nr2
        ...
        !$acc loop vector &
        !$acc private(rpf3,dlnrpf3dt9,p3,s3)
        Do k = 1, nr3
        ...
        !$acc loop vector &
        !$acc private(rpf4,dlnrpf4dt9,p4,s4)
        Do k = 1, nr4
    ...
    If ( idiag >= 5 ) Then
      Do izb = zb_lo, zb_hi
        If ( mask(izb) ) Then
          ...
          If ( iheat > 0 ) Then
            !$acc update wait(tid) &
            !$acc host(dcsect1dt9(:,izb),dcsect2dt9(:,izb),dcsect3dt9(:,izb),dcsect4dt9(:,izb), &
            !$acc      dh1dt9(:,izb),dh2dt9(:,izb),dh3dt9(:,izb),dh4dt9(:,izb))
