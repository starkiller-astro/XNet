...
Module xnet_screening
  ...
  Implicit None
  Real(dp), Allocatable, Target :: h1(:,:), h2(:,:), h3(:,:), h4(:,:)                 ! Screening factors
  Real(dp), Allocatable, Target :: dh1dt9(:,:), dh2dt9(:,:), dh3dt9(:,:), dh4dt9(:,:) ! d(screening factors)/dT9
...
Contains
  ...
  Subroutine screening_init
    ...
    If ( iscrn > 0 ) Then
      ...
      !$acc copyin(iz21,iz22,iz31,iz32,iz33,iz41,iz42,iz43,iz44, &
      !$acc        iz2c,iz3c,iz4c,zeta2w,zeta3w,zeta4w,zeta2i,zeta3i,zeta4i, &
      !$acc        ztilde,zinter,lambda0,gammae,dztildedt9)
    EndIf
    ...
    !$acc enter data async(tid) &
    !$acc copyin(h1,h2,h3,h4,dh1dt9,dh2dt9,dh3dt9,dh4dt9)
  ...
  Subroutine screening(mask_in)
    ...
    Call eos_screen2(t9t(zb_lo:zb_hi),rhot(zb_lo:zb_hi),yt(:,zb_lo:zb_hi),etae(zb_lo:zb_hi), &
      & detaedt9(zb_lo:zb_hi),ztilde(zb_lo:zb_hi),zinter(zb_lo:zb_hi),lambda0(zb_lo:zb_hi), &
      & gammae(zb_lo:zb_hi),dztildedt9(zb_lo:zb_hi))
    ...
    !$acc present(mask,nreac,t9t,h1,h2,h3,h4,dh1dt9,dh2dt9,dh3dt9,dh4dt9, &
    !$acc         zseq,zseq53,zseqi,ztilde,zinter,lambda0,gammae,dztildedt9, &
    !$acc         iz21,iz22,iz31,iz32,iz33,iz41,iz42,iz43,iz44, &
    !$acc         iz2c,iz3c,iz4c,zeta2w,zeta3w,zeta4w,zeta2i,zeta3i,zeta4i) &
    !$acc private(fhs,dfhsdt9,hw0,hi0,dlnhw0dt9,dlnhi0dt9)
    Do izb = zb_lo, zb_hi
      If ( mask(izb) ) Then

        ! Calculate screening energies as a function of Z, for prescriptions that follow this approach
        !$acc loop vector &
        !$acc private(gammaz,gammaz5,lgammaz)
        Do j = 0, izmax+2
          If ( j > 0 ) Then
        ...
        !$acc loop vector &
        !$acc private(lambda,h,hw,hi,hs,dhdt9,dhwdt9,dhidt9,dhsdt9)
        Do mu = 1, nreac(2)
    ...
    If ( idiag >= 5 ) Then
      Do izb = zb_lo, zb_hi
        If ( mask(izb) ) Then
          ...
          !$acc update wait(tid) &
          !$acc host(h1(:,izb),h2(:,izb),h3(:,izb),h4(:,izb), &
          !$acc      dh1dt9(:,izb),dh2dt9(:,izb),dh3dt9(:,izb),dh4dt9(:,izb), &
          !$acc      ztilde(izb),zinter(izb),lambda0(izb),gammae(izb),dztildedt9(izb))
          ...
          Do mu = 1, nreac(2)
            ...
            hi = hi0 * zeta2i(mu)
            hs = fhs(iz21(mu)) + fhs(iz22(mu)) - fhs(iz2c(mu))
            Write(lun_diag,"(3a5,i6,5es24.16)") &
              & 'H2',(nname(n2i(j,mu)),j=1,2),mu,lambda,h2(mu,izb),hw,hi,hs
            If ( iheat > 0 ) Then
              dhwdt9 = hw * dlnhw0dt9
              dhidt9 = hi * dlnhi0dt9
              dhsdt9 = dfhsdt9(iz21(mu)) + dfhsdt9(iz22(mu)) - dfhsdt9(iz2c(mu))
              Write(lun_diag,"(a7,2a5,i6,4es24.16)") &
          ...
          Do mu = 1, nreac(3)
            ...
            Write(lun_diag,"(4a5,i6,5es24.16)") &
              & 'H3',(nname(n3i(j,mu)),j=1,3),mu,lambda,h3(mu,izb),hw,hi,hs
            If ( iheat > 0 ) Then
              dhwdt9 = hw * dlnhw0dt9
              dhidt9 = hi * dlnhi0dt9
              dhsdt9 = dfhsdt9(iz31(mu)) + dfhsdt9(iz32(mu)) + dfhsdt9(iz33(mu)) - dfhsdt9(iz3c(mu))
              Write(lun_diag,"(a7,3a5,i6,4es24.16)") &
          ...
          Do mu = 1, nreac(4)
            ...
            hi = hi0 * zeta4i(mu)
            hs = fhs(iz41(mu)) + fhs(iz42(mu)) + fhs(iz43(mu)) + fhs(iz44(mu)) - fhs(iz4c(mu))
            Write(lun_diag,"(5a5,i6,5es24.16)") &
              & 'H4',(nname(n4i(j,mu)),j=1,4),mu,lambda,h4(mu,izb),hw,hi,hs
            If ( iheat > 0 ) Then
              dhwdt9 = hw * dlnhw0dt9
              dhidt9 = hi * dlnhi0dt9
              dhsdt9 = dfhsdt9(iz41(mu)) + dfhsdt9(iz42(mu)) + dfhsdt9(iz43(mu)) + dfhsdt9(iz44(mu)) - dfhsdt9(iz4c(mu))
              Write(lun_diag,"(a7,4a5,i6,4es24.16)") &
